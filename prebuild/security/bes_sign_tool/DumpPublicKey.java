...
class DumpPublicKey {
    ...
    static int checkRSA(RSAPublicKey key, boolean useSHA256) throws Exception {
        ...
        if (modulus.bitLength() != 2048 && modulus.bitLength() != 3072) {
            throw new Exception("Modulus should be 2048 or 3072 bits long but is " +
     ...
     */
    static String printRSA(RSAPublicKey key, boolean useSHA256, boolean raw, String binFile) throws Exception {
        ...
        int i;
        int j;
        final boolean big_endian_hex = false;

        if (binFile != null) {
            try {
                os = new FileOutputStream(binFile);
                bbuf = ByteBuffer.allocate(2048);
        ...
        BigInteger RR = R.multiply(R).mod(N);    // 2^(keyBitLength*2) mod N
        ...
        if (raw && big_endian_hex) {
            byte[] ba = N.toByteArray();
            int byte_len = N.bitLength() / 8;
            byte[] full_array = new byte[byte_len];
            i = 0;
            j = 0;

            if (byte_len > ba.length) {
                for (; i < byte_len - ba.length; i++) {
                    full_array[i] = 0;
                }
            } else if (byte_len < ba.length) {
                for (; j < ba.length - byte_len; j++) {
                    if (ba[j] != 0) {
                        throw new Exception(String.format("Byte length (%u) larger than real bit length (%u) with non-zero leading byte (j=%u 0x%02X)",
                                            ba.length, byte_len, j, ba[j]));
                    }
                }
            }
            for (; i < byte_len && j < ba.length; i++, j++) {
                full_array[i] = ba[j];
            }
            for (byte b : full_array) {
            ...
            result.append(tabpos + tabpos);
            result.append("{\n");
            result.append(tabpos + tabpos + tabpos);
            for (i = 0; i < nwords; ++i) {
    ...
    static String print(PublicKey key, boolean useSHA256, boolean rawFormat, String binFile) throws Exception {
        if (key instanceof RSAPublicKey) {
            return printRSA((RSAPublicKey) key, useSHA256, rawFormat, binFile);
        } else if (key instanceof ECPublicKey) {
    ...
    static void printUsageAndExit() {
        ...
        System.err.println("\tDumpPublicKey [-r] PubKey.pem [OutKey.bin] > source.c");
        System.exit(1);
    }

    public static void main(String[] args) {
        String keyFileName = null;
        String outBinName = null;
        ...
        try {
            ...
            check(key, true);
            System.out.print(print(key, true, rawFormat, outBinName));
        } catch (Exception e) {
            e.printStackTrace();
            System.exit(1);
        }
