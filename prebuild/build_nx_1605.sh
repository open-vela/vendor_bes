...
function build()
{
    ...
    if [ "$1" = "bootloader" ]; then
        ...
        cmd=$cmd"DSP_HIFI4_TRC_TO_MCU=1 UTILS_ESHELL_BTRF_TEST=1"

    elif [ "$1" = "bth" ]; then
        cmd=$cmd"FLASH_NV_FACTORY=1"
...
function build_hifi()
{
    ...
    rm -rf out/
    cmd="make -j T=best1605_dsp BTH_AS_MAIN_MCU=0 DSP_HIFI4_TRC_TO_MCU=1 RMT_TRACE=1 FORCE_TRACE_UART1=1 PSRAM_ENABLE=1 \
        APSRAM_ENABLE=1 XTENSA_CORE=hifi4_bes_asic_call0_abi OPENAMP_ENABLE=1 USE_BES_RPCHAN=1 RPTUN_M55C0_2_DSPC0=1"
    echo "$cmd"
    $cmd
    if [ $? -ne 0 ]; then
        exit 1
